...
//----------------------------------------------------------
// fonction pour calculer l-approximation de la dérivée de fx
//----------------------------------------------------------
double fdx(double x1)
{
  int n = 10;
  ...
  for (int i = 0; i < n; i++)
    sum1 += pow(yi[i], x1) * pow(log(yi[i]), 2);

  for (int i = 0; i < n; i++)
    sum2 += pow(yi[i], x1);

  for (int i = 0; i < n; i++)
    sum3 += pow(yi[i], x1) * log(yi[i]);

  for (int i = 0; i < n; i++)
    sum4 += 1/yi[i];

  return (sum1 * sum2) - (sum3 * sum3) + 1.0/pow(x1, 2) - (1.0/n * sum4);
};

//----------------------------------------------------------
// fonction pour calculer la valeur de la fonction
//----------------------------------------------------------
double fx(double x1)
{
  int n = 10;
...
int main(int argc,char** argv)
{
  ...
  //--------------------------------------------------------------------------------
   ...
   for(int i=0;i<length;i++) for(int j=0;j<width;j++)
    ...
    double x1 = 0.25;
    double x2 = x1 - fx(x1)/fdx(x1);
    int counter =1;

    while (fabs(x1-x2)>=tolerance && fabs(fx(x1))>=tolerance && fdx(x1)!=0 && counter <= 1000) {

      printf("iteration no    : %i\n",counter );
      printf("valeur de x1    : %f\n",x1);
      printf("valeur de x2    : %f\n",x2);
      printf("valeur de f(x2) : %f\n", fx(x2));
      printf("abs de x1 et x2 : %f\n", fabs(x1-x2));
      printf("\n");

      x2 = x1;
      x1 = x1 - fx(x1)/fdx(x1);

      if (fabs(x1-x2)>=tolerance) //  if > 0.000001 devrait etre true
        printf("1\n");
      else 
        printf("%f\n",fabs(x1-x2)); // mais c'est false, et on imprime 0.000001 ...
