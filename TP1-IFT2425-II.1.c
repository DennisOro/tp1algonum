...
int main(int argc,char** argv)
{
  ...
  for(int k=0;k<length;k++)
  {
    ...
    {
      // partie reelle
      xk = 2.0*(k-width/1.35)/(width-1);
      // partie imaginaire
      yl = 2.0*(l-length/2.0)/(length-1);
      // z_reel
      zx = 0;
      // z_imaginaire
      zy = 0;
      ...
      count = 0;
      // tant que le module de Z <= 2 et on est inferieur a 200 iterations
      while (abs((sqrt(zx*zx + zy*zy) <= 2)) && (count < maxiter)) 
      {
